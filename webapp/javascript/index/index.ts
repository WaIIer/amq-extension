...
dropZone.ondrop = function (ev: DragEvent) {
    ev.preventDefault()
}

dropZone.ondragover = function (ev: DragEvent) {
...
dropZone.ondragleave = function (ev: DragEvent) {
...
function showFile(file: File) {
    ...
    reader.onload = function (ev) {
        fileContents.appendChild(generateStatsTable(JSON.parse(ev.target.result.toString()) as RootObject));
    }
}

function parseAmqExtensionJsonFile(amqJson: JSON): string {
    var newJson = { 0: [] };
